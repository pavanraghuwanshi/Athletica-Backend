...
- `GET /api/users?page=1&limit=20` - list users visible to the authenticated account.
  ...
  - Each item also includes `lastSyncAt` and `latestRecords`, containing one latest measurement object for every health metric except ECG. Health score and status are not included in this list response.
